/*****************************************************************************
* Filename:          C:\Victory_427\Space_Invaders_Lab_4/drivers/dma_screencap_v1_00_a/src/dma_screencap.c
* Version:           1.00.a
* Description:       dma_screencap Driver Source File
...
void DMA_SCREENCAP_MasterSendWord(Xuint32 BaseAddress, Xuint32 DstAddress)
{
   ...
   */
  Xil_Out8(BaseAddress+DMA_SCREENCAP_MST_CNTL_REG_OFFSET, MST_SGWR);

  /*
   * Set user logic master address register to drive IP2Bus_Mst_Addr signal.
   */
  Xil_Out32(BaseAddress+DMA_SCREENCAP_MST_ADDR_REG_OFFSET, DstAddress);

  /*
   * Set user logic master byte enable register to drive IP2Bus_Mst_BE signal.
   */
  Xil_Out16(BaseAddress+DMA_SCREENCAP_MST_BE_REG_OFFSET, 0xFFFF);

  /*
   * Start user logic master write transfer by writting special pattern to its go port.
   */
  Xil_Out8(BaseAddress+DMA_SCREENCAP_MST_GO_PORT_OFFSET, MST_START);
}
...
void DMA_SCREENCAP_MasterRecvWord(Xuint32 BaseAddress, Xuint32 SrcAddress)
{
   ...
   */
  Xil_Out8(BaseAddress+DMA_SCREENCAP_MST_CNTL_REG_OFFSET, MST_SGRD);

  /*
   * Set user logic master address register to drive IP2Bus_Mst_Addr signal.
   */
  Xil_Out32(BaseAddress+DMA_SCREENCAP_MST_ADDR_REG_OFFSET, SrcAddress);
