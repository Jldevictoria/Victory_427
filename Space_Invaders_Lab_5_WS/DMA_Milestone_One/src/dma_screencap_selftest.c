/*****************************************************************************
* Filename:          C:\Victory_427\Space_Invaders_Lab_4/drivers/dma_screencap_v1_00_a/src/dma_screencap_selftest.c
* Version:           1.00.a
* Description:       Contains a diagnostic self-test function for the dma_screencap driver
...
#define DMA_SCREENCAP_SELFTEST_BUFSIZE  4 /* Size of buffer (n bytes) is fixed to 4 for lite Master */
#define DMA_SCREENCAP_USER_NUM_REG  5/* Number of registers in slave */

/************************** Variable Definitions ****************************/

static Xuint8 __attribute__((aligned (128))) SrcBuffer[DMA_SCREENCAP_SELFTEST_BUFSIZE];   /* Source buffer      */
static Xuint8 __attribute__((aligned (128))) DstBuffer[DMA_SCREENCAP_SELFTEST_BUFSIZE];   /* Destination buffer */
...
XStatus DMA_SCREENCAP_SelfTest(void * baseaddr_p)
{
  ...
  for (write_loop_index = 0 ; write_loop_index < DMA_SCREENCAP_USER_NUM_REG; write_loop_index++)
    DMA_SCREENCAP_mWriteSlaveReg0 (baseaddr, write_loop_index*4, (write_loop_index+1)*READ_WRITE_MUL_FACTOR);
  for (read_loop_index = 0 ; read_loop_index < DMA_SCREENCAP_USER_NUM_REG; read_loop_index++)
    if ( DMA_SCREENCAP_mReadSlaveReg0 (baseaddr, read_loop_index*4) != (read_loop_index+1)*READ_WRITE_MUL_FACTOR){
  ...
  if ( (DMA_SCREENCAP_mReadSlaveReg0 (baseaddr, 0)) != 0x0){
  ...
  * Master */
  ...
  for ( Index = 0; Index < DMA_SCREENCAP_SELFTEST_BUFSIZE; Index++ )
  {
    SrcBuffer[Index] = Index;
    DstBuffer[Index] = 0;
  }
  xil_printf("   - start user logic master module to receive one word from the source\n\r");
  DMA_SCREENCAP_MasterRecvWord(baseaddr, (Xuint32) SrcBuffer);
  while ( ! DMA_SCREENCAP_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
  xil_printf("   - start user logic master module to send onw word to the destination\n\r");
  DMA_SCREENCAP_MasterSendWord(baseaddr, (Xuint32) DstBuffer);
  while ( ! DMA_SCREENCAP_mMasterDone(baseaddr) ) {}
  xil_printf("   - transfer completed\n\r");
